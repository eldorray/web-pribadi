<x-layouts.frontend :settings="$settings" :socialLinks="$socialLinks" title="Projects | {{ $settings['site_name'] ?? 'My Portofolio' }}">

    <div class="pt-28 sm:pt-40 pb-12 sm:pb-20 px-4 sm:px-6 md:px-12 max-w-7xl mx-auto">
        <!-- Hero Section / Header -->
        <section class="mb-12 sm:mb-20">
            <h1 class="font-headline text-3xl sm:text-5xl md:text-7xl font-extrabold tracking-tight text-on-surface mb-4 sm:mb-6 max-w-4xl">
                Crafting digital <span class="text-primary italic">structures</span> with editorial precision.
            </h1>
            <p class="font-body text-base sm:text-lg text-outline leading-relaxed max-w-2xl">
                A curated selection of architectural interfaces and high-performance development projects where form follows function.
            </p>
        </section>

        <!-- Filter System -->
        <nav class="flex items-center gap-x-6 sm:gap-x-8 gap-y-3 sm:gap-y-4 mb-10 sm:mb-16 overflow-x-auto pb-4 -mx-4 px-4 sm:mx-0 sm:px-0 scrollbar-hide flex-nowrap sm:flex-wrap" id="project-filters">
                    ...
                    <div class="lg:col-span-8 group cursor-pointer">
                        <div class="bg-surface-container-lowest rounded-xl overflow-hidden shadow-ambient hover:shadow-ambient-hover transition-all duration-500">
                            ...
                            <div class="p-6 sm:p-8 md:p-10">
                                ...
                                <h3 class="font-headline text-2xl sm:text-3xl font-bold mb-4 group-hover:text-primary transition-colors">{{ $project->title }}</h3>
        ...
        <!-- CTA Section -->
        <section class="mt-20 sm:mt-32 p-8 sm:p-12 md:p-20 bg-primary-container rounded-2xl sm:rounded-3xl text-on-primary-container flex flex-col items-center text-center">
            <h2 class="font-headline text-3xl sm:text-4xl md:text-5xl font-bold mb-4 sm:mb-6 max-w-2xl">Ready to build your next structural masterpiece?</h2>
            <p class="font-body text-base sm:text-lg mb-8 sm:mb-10 opacity-80 max-w-xl">Let's collaborate to bring your vision into the digital physical realm with precision and style.</p>
            <div class="flex flex-col sm:flex-row gap-3 sm:gap-4">
                <a href="{{ route('contact') }}" class="px-8 sm:px-10 py-3.5 sm:py-4 bg-white text-primary font-headline font-bold rounded-full hover:scale-105 active:scale-95 transition-all">Start a Project</a>
                <a href="{{ route('about') }}" class="px-8 sm:px-10 py-3.5 sm:py-4 bg-primary text-white font-headline font-bold rounded-full border border-white/20 hover:scale-105 active:scale-95 transition-all">View Process</a>
            </div>
        </section>
    </div>
